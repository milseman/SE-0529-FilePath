...
// `FilePath.resolve()` is a filesystem-touching API: tests need a real OS,
...
struct ResolveTests {
  ...
  func realFileResolvesToAbsoluteSymlinkFreePath() throws {
    try withPlatform(.darwin) {
      ...
      // FilePath.init? on a Foundation URL path can only fail on NUL,
      // which these paths cannot contain — hence the bare `!`.
      let resolved = try FilePath(fileURL.path)!.resolve()
  ...
  func resolvingThroughSymlinkYieldsTarget() throws {
    try withPlatform(.darwin) {
      ...
      let resolvedLink = try FilePath(link.path)!.resolve()
      let resolvedTarget = try FilePath(target.path)!.resolve()
  ...
  func firmlinkedHomeRealPath() throws {
    try withPlatform(.darwin) {
      ...
      let resolved = try FilePath(home)!.resolve()
      let s = String(decoding: resolved)
